...
for n = 1:N_train_files
   %if mod(n,10)==1
      ...
      Features_train = [Features_train;get_feature(conf.number_of_slots,img)];
      %------------------------------------------------------------------------
      % imshow(
   %end
...
for n = 1:N_eval_files
  ...
  predicted_vector = get_feature(conf.number_of_slots,img);
...
fprintf('The accuracy rate is %.2f', accuracy);
%% The END
