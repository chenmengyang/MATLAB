...
% training_mean = zeros(5,3);
% training_variance = zeros(5,3);
% for i = 1:length(training_mean)
%    index = find(train_classes == i);
%    red = Features_train(index,1);
%    green = Features_train(index,2);
%    blue = Features_train(index,3);
%    mean_r = mean(red);
%    mean_g = mean(green);
%    mean_b = mean(blue);
%    training_mean(i,:) = [mean_r mean_g mean_b];
...
%    training_variance(i,:) = [variance_r variance_g variance_b];
% end
[training_mean, training_variance] = get_mean_variance(Features_train, train_classes);

t = tabulate(train_classes);
training_prior = t(:,3)/100;
...
gaussian_values = zeros(250,5);
for i = 1:length(Features_predicted)
    for j = 1:5
        Pj = training_prior(j);
        Pk = 1;
        for k = 1:size(Features_predicted,2)
            Pk = Pk*mvnpdf(Features_predicted(i,k),training_mean(j,k),training_variance(j,k));
        end
        gaussian_values(i,j) = Pj*Pk;
    end
end

%% Get results
[m,res] = max(gaussian_values');
result = res';
for n = 1:length(result)
    result(n) = classIDnumbers(result(n));
end
...
fprintf('The accuracy rate is %.2f', mean(double(result == eval_classes)) * 100);
%% The END
